...
namespace lue::cf {
    ...
    auto Dataset::create(std::string const& name, int const create_mode) -> Dataset
    {
        ...
        dataset.add_attribute("history", std::string{fmt::format("LUE-{}", lue::version())});
    ...
    auto Dataset::open(std::string const& name, int const open_mode) -> Dataset
    {
        ...
        {
            throw std::runtime_error(fmt::format("Dataset {} does not conform to the CF convention", name));
        }
