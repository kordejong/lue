...
namespace lue::utility {
    ...
    namespace {
            ...
            std::optional<std::string> const& default_value = {})
        {
            ...
            description.push_back(fmt::format("{}: {}", name, value));
        }
            ...
            std::optional<std::string> const& default_value = {})
        {
            ...
            description.push_back(fmt::format("{}:", name));

            for (auto const& item : split_string(value, '\n'))
            {
                description.push_back(fmt::format("- {}", item));
            }
        }
        ...
        {
            description.push_back(fmt::format("Variable: {}", variable.name()));
        ...
        {
            description.push_back(fmt::format("Group: {}", group.name()));
    ...
    auto describe_cf_dataset(cf::Dataset const& dataset) -> std::vector<std::string>
    {
        ...
        description.push_back(fmt::format("Dataset: {}", dataset.path()));
