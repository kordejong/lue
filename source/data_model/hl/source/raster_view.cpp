...
namespace lue::data_model {
    ...
    template<typename DatasetPtr>
    void RasterView<DatasetPtr>::add_layer(std::string const& name)
    {
        ...
        if (phenomenon.object_id().nr_objects() == 1)
        {
            ...
            // Space domain
            {
                if (property_set.has_space_domain())
                {
                    ...
                    if (configuration.value<Mobility>() == Mobility::stationary &&
                        configuration.value<SpaceDomainItemType>() == SpaceDomainItemType::box &&
            ...
            if (space_domain_ok)
            {
                ...
                {
                    if (properties.shape_per_object(space_discretization_property_name) ==
                            ShapePerObject::same &&
                        properties.value_variability(space_discretization_property_name) ==
                            ValueVariability::constant)
                    {
                        assert(properties.collection<same_shape::Properties>().contains(
                            space_discretization_property_name));

                        auto const& discretization_property{
                            properties
                                .collection<same_shape::Properties>()[space_discretization_property_name]};
                        auto const& value{discretization_property.value()};

                        if (!discretization_property.time_is_discretized() &&
                            !discretization_property.space_is_discretized() && value.nr_arrays() == 1 &&
                            value.array_shape() == hdf5::Shape{2} &&
                            value.file_datatype() ==
                                hdf5::Datatype{
                                    hdf5::StandardDatatypeTraits<hdf5::Shape::value_type>::type_id()})
                        {
    ...
    namespace constant {
        ...
        */
            ...
            typename RasterView<DatasetPtr>::Layer
        {
            ...
            raster_property.value().expand(this->object_id(), this->grid_shape(), no_data_value);
            ...
            std::string const& layer_name) -> std::tuple<ID, hdf5::Shape, hdf5::Datatype>
        {
            ...
            return std::make_tuple(raster_view.object_id(), raster_view.grid_shape(), layer.file_datatype());
        }
        ...
        */
            ...
            typename data_model::RasterView<DatasetPtr>::SpaceBox const& space_box) -> RasterView<DatasetPtr>
        {
            ...
            SpaceConfiguration space_configuration{Mobility::stationary, SpaceDomainItemType::box};
            ...
            auto& property_set = phenomenon.property_sets().add(
                property_set_name, space_configuration, hdf5::native_float64, 2);

            // Space domain
            auto space_domain_value{property_set.space_domain().value<StationarySpaceBox>()};
    ...
    namespace variable {
        ...
        */
...

        {
            ...
            auto& time_discretization_property =
                properties
                    .collection<same_shape::constant_shape::Properties>()[time_discretization_property_name];
            ...
            // Scan for properties that represent the rasters
            {
                ...
                for (auto const& name : raster_properties.names())
                {
                    ...
                    bool const property_is_discretized =
                        ...
                        property.time_discretization_type() == TimeDiscretization::regular_grid &&
                        property.space_is_discretized() &&
                        property.space_discretization_type() == SpaceDiscretization::regular_grid;
                    ...
                    if (property_is_discretized)
                    {
                        ...
                        bool const time_discretization_properties_match =
                            property.time_discretization_property().id() == time_discretization_property.id();
                        bool const space_discretization_properties_match =
                            property.space_discretization_property().id() ==
                            space_discretization_property.id();

                        if (time_discretization_properties_match && space_discretization_properties_match)
                        {
            ...
            typename RasterView<DatasetPtr>::Layer
        {
            ...
            auto& time_discretization_property = properties.template collection<
                same_shape::constant_shape::Properties>()[time_discretization_property_name];
            auto& space_discretization_property =
                properties.template collection<same_shape::Properties>()[space_discretization_property_name];
            ...
            if (data_model::contains_raster(dataset, phenomenon_name, property_set_name))
            {
                ...
                // Time domain
                {
                    if (property_set.has_time_domain())
                    {
                        ...
                        if (configuration.value<TimeDomainItemType>() == TimeDomainItemType::box)
                        {
                            // NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
                            auto time_box = const_cast<TimeDomain&>(time_domain).value<TimeBox>();
                ...
                if (time_domain_ok)
                {
                    ...
                    if (properties.contains(time_discretization_property_name))
                    {
                        if (properties.shape_per_object(time_discretization_property_name) ==
                                ShapePerObject::same &&
                            properties.value_variability(time_discretization_property_name) ==
                                ValueVariability::variable &&
                            properties.shape_variability(time_discretization_property_name) ==
                                ShapeVariability::constant)
                        {
                            assert(properties.collection<same_shape::constant_shape::Properties>().contains(
                                time_discretization_property_name));

                            auto const& discretization_property{properties.collection<
                                same_shape::constant_shape::Properties>()[time_discretization_property_name]};
                            auto const& value{discretization_property.value()};

                            if (!discretization_property.time_is_discretized() &&
                                !discretization_property.time_is_discretized() && value.nr_arrays() == 1 &&
        ...
        */
            ...
            typename data_model::RasterView<DatasetPtr>::SpaceBox const& space_box) -> RasterView<DatasetPtr>
        {
            ...
            TimeConfiguration time_configuration{TimeDomainItemType::box};
            SpaceConfiguration space_configuration{Mobility::stationary, SpaceDomainItemType::box};

            static_assert(std::is_same_v<typename RasterView<DatasetPtr>::SpaceBox::value_type, double>);
            auto& property_set = phenomenon.property_sets().add(
                property_set_name, time_configuration, clock, space_configuration, hdf5::native_float64, 2);
...

            {
                ...
                auto& property = properties.add<same_shape::constant_shape::Property>(
                    time_discretization_property_name, shape_datatype, lue::hdf5::Shape{1});  // nr_time_steps
                auto& value = property.value();
                value.expand(1);
