        ...
        public:
                ...
                std::string const& value) -> Attribute
            {
                    ...
                    status != NC_NOERR)
                {
                    throw std::runtime_error(
                        fmt::format("Cannot add attribute {}: {}", name, error_message(status)));
                }
            ...
            [[nodiscard]] auto value(std::optional<std::string> default_value = {}) const -> std::string
            {
                    ...
                    status != NC_NOERR)
                {
                    ...
                    throw std::runtime_error(
                        fmt::format("Cannot get attribute length: {}", error_message(status)));
                }
                ...
                {
                    throw std::runtime_error(
                        fmt::format("Cannot obtain attribute value {}: {}", _name, error_message(status)));
                }
            ...
            auto value() const -> T
            {
                ...
                if (length != 1)
                {
                    throw std::runtime_error(fmt::format("Attribute value {} is not scalar", _name));
                }
