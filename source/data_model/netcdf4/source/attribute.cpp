...
namespace lue::netcdf {
    ...
    auto Attribute::open(int const group_id, int const variable_id, int attribute_idx) -> Attribute
    {
        ...
        if (int status = nc_inq_attname(group_id, variable_id, attribute_idx, buffer.data());
            status != NC_NOERR)
        {
            throw std::runtime_error(fmt::format("Cannot get attribute name: {}", error_message(status)));
        }
    ...
    auto Attribute::type() const -> nc_type
    {
        ...
        {
            throw std::runtime_error(fmt::format("Cannot get attribute type: {}", error_message(status)));
        }
    ...
    auto Attribute::length() const -> std::size_t
    {
        ...
        {
            throw std::runtime_error(fmt::format("Cannot get attribute length: {}", error_message(status)));
        }
