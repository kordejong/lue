...
namespace lue::utility {
    namespace {
        ...
        auto read_gdal_raster(std::string const& pathname, Collection& collection) -> hdf5::Shape
        {
            // Open dataset
            ...
            return {
                static_cast<hdf5::Shape::value_type>(nr_rows), static_cast<hdf5::Shape::value_type>(nr_cols)};
        }
            ...
            nlohmann::json const& object_tracker_json, data_model::ObjectTracker& object_tracker)
        {
            if (contains(object_tracker_json, "active_set_index"))
            {
                ...
                std::ranges::transform(
                    idxs, idxs.begin(), [offset](auto const idx) -> auto { return idx + offset; });
        ...
        auto read_space_box(std::string const& dataset_name) -> std::vector<Coordinate>
        {
            ...
            data_model::different_shape::variable_shape::Properties& properties)
        {
            ...
            {
                object_arrays[object_id[object_idx]].write(values[object_idx].data());
            }
        }
            ...
            nlohmann::json const& property_set_json, data_model::PropertySets& property_sets)
        {
            ...
            else
            {
                ...
                if (epoch_json.contains("origin"))
                {
                    ...
                    {
                        auto const epoch_calendar = data_model::string_to_aspect<data_model::time::Calendar>(
                ...
                data_model::time::DurationCount const tick_period_count = clock_json.at("tick_period_count");
                std::string const unit = clock_json.at("unit");

                // FIXME Read the time domain item type from the json
                data_model::TimeConfiguration const time_configuration{data_model::TimeDomainItemType::point};
                data_model::Clock const clock{
        ...
        Metadata const& /* metadata */)
    {
        ...
        translate_json_to_lue(lue_json, lue_pathname, add);
    }
