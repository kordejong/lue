...
namespace lue {
    namespace value_policies {
        ...
        */
        auto focal_minimum(auto const& field, auto const& kernel) -> api::Field
        {
            api::detail::unsupported_overload("focal_minimum", field, kernel);
    ...
    namespace api {
        ...
        /*!
            @brief      TODO
        */
        auto focal_minimum(Field const& field, BooleanKernel const& kernel) -> Field
        {
            return std::visit(
                overload{
                    [](auto const& field, auto const& kernel) -> Field
                    { return value_policies::focal_minimum(field, kernel); }},
                field.variant(),
                kernel.variant());
        }
