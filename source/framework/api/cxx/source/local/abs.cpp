#include "lue/framework/api/cxx/local/abs.hpp"
#include "lue/framework/api/cxx/detail/unsupported_overload.hpp"
#include "lue/framework/api/cxx/detail/overload.hpp"
#include "lue/framework/algorithm/value_policies/abs.hpp"


namespace lue {
    namespace value_policies {

        auto abs(auto const& field) -> api::Field
        {
            api::detail::unsupported_overload("abs", field);

            return {};
        }

    }  // namespace value_policies


    namespace api {

        auto abs(Field const& field) -> Field
        {
            return std::visit(
                overload{[](auto const& field) -> Field { return value_policies::abs(field); }},
                field.variant());
        }

    }  // namespace api
}  // namespace lue
