#define BOOST_TEST_MODULE lue framework api cxx local trunc
#include "lue/framework/api/cxx/local/trunc.hpp"
#include "unit_test.hpp"
#include "lue/framework.hpp"


BOOST_AUTO_TEST_CASE(trunc_array)
{
    ...
    {
        lue::api::Field result = lue::api::trunc(array);
...
BOOST_AUTO_TEST_CASE(trunc_scalar)
{
    using Element = lue::SmallestFloatingPointElement;
    ...
    {
        lue::api::Field result = lue::api::trunc(scalar);
...
BOOST_AUTO_TEST_CASE(trunc_value)
{
    using Element = lue::SmallestFloatingPointElement;
    using Scalar = lue::Scalar<Element>;
    ...
    {
        lue::api::Field result = lue::api::trunc(value);
