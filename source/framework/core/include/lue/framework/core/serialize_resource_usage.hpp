...
namespace lue::detail::root {
    ...
    */
    ...
    /*!
        @brief      Return the number of times the resource has already been used
        @return     A use count >= 0
    */
    template<std::equality_comparable Key>
    auto current_resource_use_count(ResourceUseCountByKey<Key>& use_count_by, Key const& key) -> Count
    {
        lue_hpx_assert(on_root_locality());

        return use_count_by.contains(key) ? use_count_by.at(key) : 0;
    }
    ...
    */
    template<std::equality_comparable Key>
    auto current_resource_use_count(Key const& key) -> Count
    {
        return current_resource_use_count(detail::resource_use_count_by<Key>(), key);
    }
    ...
    /*!
        ...
        We use a shared future here because multiple future (in time) calls may depend on the same previous
        call.
    */
    template<std::equality_comparable Key>
    ...
    */
    ...
    void add_resource_use_finished(
        ResourceUseFinished<Key>& use_finished, Key const& key, Count const count, hpx::future<void> future)
    {
        lue_hpx_assert(on_root_locality());
    ...
    */
    ...
    {
        return add_resource_use_finished(detail::resource_use_finished<Key>(), key, count, std::move(future));
    }
